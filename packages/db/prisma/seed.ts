...
async function main() {
  ...
  console.log(`Currency: ${eur.code}`);

  // ── Support: Units ───────────────────────────────────────────────
  const units = [
    { name: "Kilogramas", abbreviation: "kg" },
    { name: "Gramas", abbreviation: "g" },
    { name: "Litras", abbreviation: "l" },
    { name: "Mililitras", abbreviation: "ml" },
    { name: "Vienetas", abbreviation: "vnt" },
  ];
  const unitMap: Record<string, string> = {};
  for (const u of units) {
    const created = await prisma.unit.upsert({
      where: { id: u.abbreviation },
      update: {},
      create: { id: u.abbreviation, name: u.name, abbreviation: u.abbreviation },
    });
    unitMap[u.abbreviation] = created.id;
  }
  ...
  if (!barboraSource) {
    barboraSource = await prisma.source.create({
      data: { code: "BARBORA", name: "Barbora", type: "ONLINE_GROCERY" },
    });
  ...
  const existingPolicy = await prisma.userRecommendationPolicy.findFirst({
    where: { userId: demoUser.id, canonicalCategoryId: null, mode: "explore_allowed" },
  });
  if (!existingPolicy) {
